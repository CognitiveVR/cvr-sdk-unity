...
namespace Cognitive3D.UnityGLTF.Extensions
{
	public static class SchemaExtensions
	{
		...
		public static void RSetUnityTransform(this Node node, Transform transform)
		{
			node.Translation = transform.localPosition.RToGltfVector3Convert();
			node.Rotation = transform.localRotation.RToGltfQuaternionConvert();
			node.Scale = transform.localScale.RToGltfVector3Raw();
		}
		...
		public static void RGetTRSProperties(this Matrix4x4 mat, out Vector3 position, out Quaternion rotation,
			out Vector3 scale)
		{
		...
		public static void GetTRSProperties(this GLTF.Math.Matrix4x4 gltfMat, out Vector3 position, out Quaternion rotation,
			out Vector3 scale)
		{
			gltfMat.RToUnityMatrix4x4Convert().RGetTRSProperties(out position, out rotation, out scale);
		}
		...
		/// <returns></returns>
		public static GLTF.Math.Vector4 RGetColumn(this GLTF.Math.Matrix4x4 mat, uint columnNum)
		{
			switch (columnNum)
			{
				case 0:
		...
		public static Quaternion ToUnityQuaternionConvert(this GLTF.Math.Quaternion gltfQuat)
		{
			...
			Vector3 newAxis = axisFlipScale * Vector3.Scale(origAxis, CoordinateSpaceConversionScale.RToUnityVector3Raw());
		...
		public static GLTF.Math.Quaternion RToGltfQuaternionConvert(this Quaternion unityQuat)
		{
		...
		public static Matrix4x4 RToUnityMatrix4x4Convert(this GLTF.Math.Matrix4x4 gltfMat)
		{
			Matrix4x4 rawUnityMat = gltfMat.RToUnityMatrix4x4Raw();
			Vector3 coordinateSpaceConversionScale = CoordinateSpaceConversionScale.RToUnityVector3Raw();
		...
		public static GLTF.Math.Matrix4x4 ToGltfMatrix4x4Convert(this Matrix4x4 unityMat)
		{
			Vector3 coordinateSpaceConversionScale = CoordinateSpaceConversionScale.RToUnityVector3Raw();
			Matrix4x4 convert = Matrix4x4.Scale(coordinateSpaceConversionScale);
			GLTF.Math.Matrix4x4 gltfMat = (convert * unityMat * convert).RToGltfMatrix4x4Raw();
			return gltfMat;
		}
		...
		{
			Vector3 coordinateSpaceConversionScale = CoordinateSpaceConversionScale.RToUnityVector3Raw();
			Vector3 unityVec3 = Vector3.Scale(gltfVec3.RToUnityVector3Raw(), coordinateSpaceConversionScale);
			return unityVec3;
		}
		...
		public static GLTF.Math.Vector3 RToGltfVector3Convert(this Vector3 unityVec3)
		{
			Vector3 coordinateSpaceConversionScale = CoordinateSpaceConversionScale.RToUnityVector3Raw();
			GLTF.Math.Vector3 gltfVec3 = Vector3.Scale(unityVec3, coordinateSpaceConversionScale).RToGltfVector3Raw();
			return gltfVec3;
		}

		public static GLTF.Math.Vector3 RToGltfVector3Raw(this Vector3 unityVec3)
		{
			GLTF.Math.Vector3 gltfVec3 = new GLTF.Math.Vector3(unityVec3.x, unityVec3.y, unityVec3.z);
			return gltfVec3;
		}

		public static GLTF.Math.Vector4 RToGltfVector4Raw(this Vector4 unityVec4)
		{
			GLTF.Math.Vector4 gltfVec4 = new GLTF.Math.Vector4(unityVec4.x, unityVec4.y, unityVec4.z, unityVec4.w);
			return gltfVec4;
		}

		public static Matrix4x4 RToUnityMatrix4x4Raw(this GLTF.Math.Matrix4x4 gltfMat)
		{
			Vector4 rawUnityCol0 = gltfMat.RGetColumn(0).RToUnityVector4Raw();
			Vector4 rawUnityCol1 = gltfMat.RGetColumn(1).RToUnityVector4Raw();
			Vector4 rawUnityCol2 = gltfMat.RGetColumn(2).RToUnityVector4Raw();
			Vector4 rawUnityCol3 = gltfMat.RGetColumn(3).RToUnityVector4Raw();
		...
		public static GLTF.Math.Matrix4x4 RToGltfMatrix4x4Raw(this Matrix4x4 unityMat)
		{
			GLTF.Math.Vector4 c0 = unityMat.GetColumn(0).RToGltfVector4Raw();
			GLTF.Math.Vector4 c1 = unityMat.GetColumn(1).RToGltfVector4Raw();
			GLTF.Math.Vector4 c2 = unityMat.GetColumn(2).RToGltfVector4Raw();
			GLTF.Math.Vector4 c3 = unityMat.GetColumn(3).RToGltfVector4Raw();
		...
		public static Vector2 RToUnityVector2Raw(this GLTF.Math.Vector2 vec2)
		{
			return new Vector2(vec2.X, vec2.Y);
		}

		public static Vector2[] RToUnityVector2Raw(this GLTF.Math.Vector2[] inVecArr)
		{
			Vector2[] outVecArr = new Vector2[inVecArr.Length];
			for (int i = 0; i < inVecArr.Length; ++i)
			{
				outVecArr[i] = inVecArr[i].RToUnityVector2Raw();
			}
			return outVecArr;
		}

		public static Vector3 RToUnityVector3Raw(this GLTF.Math.Vector3 vec3)
		{
			return new Vector3(vec3.X, vec3.Y, vec3.Z);
		}

		public static Vector3[] RToUnityVector3Raw(this GLTF.Math.Vector3[] inVecArr)
		{
			Vector3[] outVecArr = new Vector3[inVecArr.Length];
			for (int i = 0; i < inVecArr.Length; ++i)
			{
				outVecArr[i] = inVecArr[i].RToUnityVector3Raw();
			}
			return outVecArr;
		}

		public static Vector4 RToUnityVector4Raw(this GLTF.Math.Vector4 vec4)
		{
			return new Vector4(vec4.X, vec4.Y, vec4.Z, vec4.W);
		}

		public static Vector4[] RToUnityVector4Raw(this GLTF.Math.Vector4[] inVecArr)
		{
			Vector4[] outVecArr = new Vector4[inVecArr.Length];
			for (int i = 0; i < inVecArr.Length; ++i)
			{
				outVecArr[i] = inVecArr[i].RToUnityVector4Raw();
			}
			return outVecArr;
		}

		public static UnityEngine.Color RToUnityColorRaw(this GLTF.Math.Color color)
		{
			return new UnityEngine.Color(color.R, color.G, color.B, color.A);
		}

		public static GLTF.Math.Color RToNumericsColorRaw(this UnityEngine.Color color)
		{
			return new GLTF.Math.Color(color.r, color.g, color.b, color.a);
		}

		public static UnityEngine.Color[] RToUnityColorRaw(this GLTF.Math.Color[] inColorArr)
		{
			UnityEngine.Color[] outColorArr = new UnityEngine.Color[inColorArr.Length];
			for (int i = 0; i < inColorArr.Length; ++i)
			{
				outColorArr[i] = inColorArr[i].RToUnityColorRaw();
			}
			return outColorArr;
		}
		...
		public static Matrix4x4 RToUnityMatrix4x4(this GLTF.Math.Matrix4x4 matrix)
		{
		...
		public static Matrix4x4[] ToUnityMatrix4x4(this GLTF.Math.Matrix4x4[] inMatrixArr)
		{
			...
			{
				outMatrixArr[i] = inMatrixArr[i].RToUnityMatrix4x4();
			}
			return outMatrixArr;
		}
	}
