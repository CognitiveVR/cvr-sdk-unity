...
namespace Cognitive3D
{
    ...
    {
        public static bool TryGetCurrentWorldGazeRay(ref Ray ray)
        {
            if (!GameplayReferences.HMD) return false;

            ray.origin = GameplayReferences.HMD.position;

            return TryGetLookDirection(ref ray);
        }

        [Obsolete("Use TryGetCurrentWorldGazeRay() instead")]
        public static Ray GetCurrentWorldGazeRay()
        {
            Ray ray = new Ray();

            if (!GameplayReferences.HMD) return ray;
            TryGetLookDirection(ref ray);
            ray.origin = GameplayReferences.HMD.position;
            return ray;
        }
#if C3D_SRANIPAL
        ...
        static bool TryGetLookDirection(ref Ray ray)
        {
            InitCheck();
            var tempray = new Ray();
            if (version == ViveSR.anipal.Eye.SRanipal_Eye_Framework.SupportedEyeVersion.version1)
            {
                if (ViveSR.anipal.Eye.SRanipal_Eye.GetGazeRay(ViveSR.anipal.Eye.GazeIndex.COMBINE, out tempray))
                {
                    lastDir = GameplayReferences.HMD.TransformDirection(tempray.direction);
                    ray.direction = lastDir;
                    retun true;
                }
            }
            else
            {
                if (ViveSR.anipal.Eye.SRanipal_Eye_v2.GetGazeRay(ViveSR.anipal.Eye.GazeIndex.COMBINE, out tempray))
                {
                    lastDir = GameplayReferences.HMD.TransformDirection(tempray.direction);
                    ray.direction = lastDir;
                    retun true;
        ...
        bool lastDirectionIsValid = false;
        ...
        {        
            ...
            {
                ray.direction = Cognitive3D.GameplayReferences.HMD.forward;
                return true;
            }
        }
            ...
            if (centereye.TryGetFeatureValue(UnityEngine.XR.CommonUsages.eyesData, out eyes))
            {
                ...
                if (eyes.TryGetFixationPoint(out convergencePoint))
                {
                    ...
                    if (convergencePoint != Vector3.zero)
                    {
                        ...
                        ray.direction = worldGazeDirection;
                        return true;
                    }
                }
                return false;
